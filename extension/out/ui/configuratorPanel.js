...
exports.ConfiguratorPanel = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
class ConfiguratorPanel {
    static createOrShow(extensionUri, dbtProjectManager) {
        const column = vscode.window.activeTextEditor
            ? vscode.window.activeTextEditor.viewColumn
            : undefined;
        // If we already have a panel, show it
        // For simplicity, we'll create a new one each time
        const panel = vscode.window.createWebviewPanel(ConfiguratorPanel.viewType, 'DBT Configurator', column || vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    constructor(panel, extensionUri, dbtProjectManager) {
        this._disposables = [];
        this.currentLayer = 'bronze';
        this._panel = panel;
    ...
    async _generateSilver(data) {
        try {
            ...
            const pythonModule = require('../../../utils/dbt_generator');
            const sqlContent = pythonModule.generate_dbt_model(data.sourceSchema, data.sourceTable, data.resultTable, 'silver');
            const sourcesYml = pythonModule.generate_sources_yml(data.sourceSchema, data.sourceTable, data.resultTable);
            const schemaYml = pythonModule.generate_schema_yml(data.resultTable, 'silver');
            // Save files
            ...
            vscode.window.showInformationMessage(`✅ Silver model created: models/silver/${data.resultTable}.sql`);
            // Open the SQL file
    ...
    async _generateGold(data) {
        try {
            ...
            // Use Python backend
            const pythonModule = require('../../../utils/dbt_generator');
            const sqlContent = pythonModule.generate_dbt_model(data.sourceSchema, data.sourceTable, data.resultTable, 'gold');
            const schemaYml = pythonModule.generate_schema_yml(data.resultTable, 'gold');
            // Save files
            ...
            vscode.window.showInformationMessage(`✅ Gold model created: models/gold/${data.resultTable}.sql`);
            // Open the SQL file
            const doc = await vscode.workspace.openTextDocument(sqlPath);
            await vscode.window.showTextDocument(doc);
        }
    ...
    async _openFolderInExplorer(folderPath) {
        try {
...
ConfiguratorPanel.viewType = 'dbtConfiguratorView';
//# sourceMappingURL=configuratorPanel.js.map